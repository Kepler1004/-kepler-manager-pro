...
type Row = {
  ...
  report_enabled: boolean; whatsapp_number: string | null; report_send_hour: number;
};
...
export function ParticipantManager() {
  ...
  async function save() {
    ...
    const res = await fetch('/api/report-participants', {
      ...
      body: JSON.stringify({
        updates: rows.map((r) => ({
          id: r.id, enabled: r.report_enabled,
          whatsapp_number: r.whatsapp_number, send_hour: r.report_send_hour,
        })),
      }),
    });
    setBusy(false);
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-xl rounded-xl bg-white p-4 shadow-xl">
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <div key={r.id} className="flex items-center gap-2 rounded px-2 py-1.5 hover:bg-gray-50">
                    <input type="checkbox" checked={r.report_enabled} onChange={() => upd(r.id, { report_enabled: !r.report_enabled })} className="h-4 w-4 shrink-0" />
                    <span className="w-24 shrink-0 truncate text-sm text-gray-800">{r.full_name || t('no_name')}</span>
                    <input
                      value={r.whatsapp_number ?? ''}
                      onChange={(e) => upd(r.id, { whatsapp_number: e.target.value })}
                      placeholder="60198103659"
                      className="w-36 rounded border px-2 py-1 text-sm"
                    />
                    <select
                      value={r.report_send_hour}
                      onChange={(e) => upd(r.id, { report_send_hour: Number(e.target.value) })}
            ...
            <p className="mt-2 text-[11px] text-gray-400">번호: 국가코드 포함 숫자만 · 시각: 말레이시아 기준 발송 시각</p>
