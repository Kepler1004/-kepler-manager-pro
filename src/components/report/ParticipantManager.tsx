      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white p-4 shadow-xl">
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <div key={r.id} className="flex items-center gap-2 rounded px-2 py-1.5 hover:bg-gray-50">
                    ...
                    <span className="w-28 shrink-0 truncate text-sm text-gray-800">{r.full_name || t('no_name')}</span>
                    <input
            ...
            <p className="mt-2 text-[11px] text-gray-400">번호: 국가코드 포함 숫자만 · 발송은 매일 18시(공휴일·비근무일 제외)</p>
