...
export interface ClassRow {
  ...
  day_of_week?: number; start_time?: string; end_time?: string; sessions_per_week?: number;
...
export default function ClassForm({ initial, subjects, teachers, onDone }:
  ...
  return (
    <form action={action} className="grid grid-cols-3 gap-3 rounded-xl border border-slate-200 bg-white p-4">
      ...
      <label className="block"><span className="lbl">{tc('day')}</span>
        <select name="day_of_week" defaultValue={String(initial?.day_of_week ?? 1)} className="inp">
      ...
      <label className="block"><span className="lbl">{t('sessions_week')}</span>
        <input name="sessions_per_week" type="number" min={1} defaultValue={initial?.sessions_per_week ?? 1} className="inp" /></label>
